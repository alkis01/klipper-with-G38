...
class PrinterProbe:
    def __init__(self, config, mcu_probe_name='probe1'):
        ...
        self.printer.lookup_object('pins').register_chip('probe1', self)
        ...
        self.gcode.register_command('PROBE1', self.cmd_PROBE1,
    ...
    def _handle_homing_move_begin(self, hmove):
        if self.mcu_probe_name in hmove.get_mcu_endstops():
            self.mcu_probe_name.probe_prepare(hmove)
    def _handle_homing_move_end(self, hmove):
        if self.mcu_probe_name in hmove.get_mcu_endstops():
            self.mcu_probe_name.probe_finish(hmove)
    def _handle_home_rails_begin(self, homing_state, rails):
        endstops = [es for rail in rails for es, name in rail.get_endstops()]
        if self.mcu_probe_name in endstops:
    ...
    def multi_probe_begin(self):
        self.mcu_probe_name.multi_probe_begin()
        self.multi_probe_pending = True
    def multi_probe_end(self):
        if self.multi_probe_pending:
            self.multi_probe_pending = False
            self.mcu_probe_name.multi_probe_end()
    def setup_pin(self, pin_type, pin_params):
        if pin_type != 'endstop' or pin_params['pin'] != 'z_virtual_endstop':
            raise pins.error("Probe virtual endstop only useful as endstop pin")
        if pin_params['invert'] or pin_params['pullup']:
            raise pins.error("Can not pullup/invert probe virtual endstop")
        return self.mcu_probe_name
    ...
    def _probe(self, speed):
        ...
        self.gcode.respond_info("probe1 at %.3f,%.3f is z=%.6f"
    ...
    def cmd_QUERY_PROBE(self, gcmd):
        ...
        res = self.mcu_probe_name.query_endstop(print_time)
        self.last_state = res
